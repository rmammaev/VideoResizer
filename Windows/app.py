...
# -*- coding: utf-8 -*-
"""
Video Resizer v4 — pywebview + HTML/CSS/JS UI.
Windows 10/11 build (EdgeChromium WebView2 + FFmpeg).
...
# Windows: добавим в PATH типичные папки FFmpeg и Python
if sys.platform == "win32":
    ...
    for _p in _win_extras:
        if _p and _p not in _cur and os.path.isdir(_p):
            _cur.insert(0, _p)
    os.environ["PATH"] = os.pathsep.join(filter(None, _cur))
...
class ResizeWorker(threading.Thread):
    ...
    def _burn_subs(self, src_path, dst_path, total_us, job_idx=1, job_total=1):
        ...
        _ass_simple = os.path.join(tempfile.gettempdir(), f"vr_bs_{id(self) & 0xFFFF:04x}.ass")
...
"""
    ...
    def _find_python():
        ...
        # Walk common Windows and macOS Python paths
        _la = os.environ.get("LOCALAPPDATA", "")
        candidates = [
            # Windows — user install
            os.path.join(_la, "Programs", "Python", "Python313", "python.exe"),
            os.path.join(_la, "Programs", "Python", "Python312", "python.exe"),
            os.path.join(_la, "Programs", "Python", "Python311", "python.exe"),
            os.path.join(_la, "Programs", "Python", "Python310", "python.exe"),
...
class SubtitleBurnWorker(threading.Thread):
    ...
    def run(self):
        ...
        ass_path = os.path.join(tempfile.gettempdir(), f"vr_burn_{id(self) & 0xFFFF:04x}.ass")
        try:
...
class TTSWorker(threading.Thread):
    ...
    # macOS say voices (used only on Darwin)
    SAY_VOICES = {
        "ru": {"female": "Milena",   "male": "Milena"},
        "en": {"female": "Samantha", "male": "Alex"},
    }
    # Windows SAPI voice keywords (substring match against available voices)
    WIN_VOICE_KEYWORDS = {
        "ru": {"female": ["irina", "ru-ru", "russian"],   "male": ["pavel", "ru-ru", "russian"]},
        "en": {"female": ["zira", "jenny", "female"],     "male": ["david", "james", "male"]},
    }
    ...
    def _say_tts(self, out_path):
        """Offline TTS: macOS say or Windows pyttsx3/SAPI."""
        if sys.platform == "darwin":
            return self._say_tts_macos(out_path)
        elif sys.platform == "win32":
            return self._say_tts_windows(out_path)
        return False

    def _say_tts_macos(self, out_path):
        try:
    ...
    def _say_tts_windows(self, out_path):
        """Windows offline TTS via pyttsx3 → WAV → FFmpeg → M4A."""
        try:
            import pyttsx3
            wav_path = out_path.replace(".m4a", ".wav")
            engine = pyttsx3.init()
            # Set rate
            engine.setProperty("rate", int(175 * self.speed))
            # Pick voice by keyword
            keywords = self.WIN_VOICE_KEYWORDS.get(self.lang, self.WIN_VOICE_KEYWORDS["en"]).get(self.gender, [])
            voices = engine.getProperty("voices")
            chosen = None
            for kw in keywords:
                for v in voices:
                    if kw.lower() in (v.name or "").lower() or kw.lower() in (v.id or "").lower():
                        chosen = v.id; break
                if chosen: break
            if chosen:
                engine.setProperty("voice", chosen)
            engine.save_to_file(self.text, wav_path)
            engine.runAndWait()
            if not os.path.isfile(wav_path):
                return False
            r = subprocess.run(
                ["ffmpeg", "-y", "-i", wav_path, "-c:a", "aac", "-b:a", "128k", out_path],
                capture_output=True, timeout=60)
    ...
    def run(self):
        ...
        out_path = os.path.join(_tmp, f"vr_tts_{id(self) & 0xFFFF:04x}.m4a")
        ok = False
    ...
    def play_audio(self, path):
        ...
        try:
            if sys.platform == "darwin":
                ...
                # Use PowerShell WindowsMediaPlayer
                ps_cmd = (
                    f"$mp = New-Object System.Windows.Media.MediaPlayer;"
                    f"$mp.Open([uri]'{str(path).replace(chr(39), '')}');"
                    f"$mp.Play(); Start-Sleep -Milliseconds 100; "
                    f"while ($mp.NaturalDuration.HasTimeSpan -and "
                    f"$mp.Position -lt $mp.NaturalDuration.TimeSpan) {{ Start-Sleep -s 1 }}"
                )
                subprocess.Popen(
                    ["powershell", "-NoProfile", "-Command",
                     "Add-Type -AssemblyName presentationcore; " + ps_cmd],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
...
function switchTab(tab) {
  ...
  // Скрываем кнопки СТАРТ/СТОП для вкладки субтитров
  const actions = document.querySelector(".actions");
  if (actions) actions.style.display = (tab === "subs" || tab === "sound") ? "none" : "";
...
function renderActiveTab() {
  ...
  bindEvents();
  if (state.active_tab === "subs" || state.active_tab === "sound") { updatePreview(); return; }
  renderFileList();
  renderChips();
  renderModeTabs();
  renderPackshotRows();
